...
test_get_system_information_sync (void)
{
    ...
    g_assert_no_error (error);
    g_assert_nonnull (info);
    g_assert_cmpstr (snapd_system_information_get_architecture (info), ==,
                     "amd64");
    ...
    g_assert_cmpstr (snapd_system_information_get_version (info), ==,
                     "VERSION");
    ...
    g_assert_true (
        date_matches (snapd_system_information_get_refresh_next (info), 2018,
                      1, 19, 13, 14, 15));
...
static void
system_information_cb (GObject *object,
                       GAsyncResult *result,
                       gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    g_autoptr (SnapdSystemInformation) info
        = snapd_client_get_system_information_finish (SNAPD_CLIENT (object),
                                                      result, &error);
    g_assert_no_error (error);
    g_assert_nonnull (info);
    g_assert_cmpint (snapd_system_information_get_confinement (info), ==,
...
test_get_system_information_async (void)
{
    ...
    snapd_client_get_system_information_async (
        client, NULL, system_information_cb, async_data_new (loop, snapd));
...
test_get_system_information_refresh (void)
{
    ...
    g_assert_cmpstr (snapd_system_information_get_refresh_timer (info), ==,
                     "00:00~24:00/4");
    g_assert_true (
        date_matches (snapd_system_information_get_refresh_hold (info), 2018,
                      1, 20, 1, 2, 3));
    g_assert_true (
        date_matches (snapd_system_information_get_refresh_last (info), 2018,
                      1, 19, 1, 2, 3));
    g_assert_true (
        date_matches (snapd_system_information_get_refresh_next (info), 2018,
                      1, 19, 13, 14, 15));
...
test_login_sync (void)
{
    ...
    g_autoptr (SnapdUserInformation) user_information
        = snapd_client_login2_sync (client, "test@example.com", "secret", NULL,
                                    NULL, &error);
    g_assert_no_error (error);
...
test_login_async (void)
{
    ...
    snapd_client_login2_async (client, "test@example.com", "secret", NULL,
                               NULL, login_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
test_login_invalid_email (void)
{
    ...
    g_autoptr (SnapdUserInformation) user_information
        = snapd_client_login2_sync (client, "not-an-email", "secret", NULL,
                                    NULL, &error);
...
test_login_invalid_password (void)
{
    ...
    g_autoptr (SnapdUserInformation) user_information
        = snapd_client_login2_sync (client, "test@example.com", "invalid",
                                    NULL, NULL, &error);
...
test_login_otp_invalid (void)
{
    ...
    g_autoptr (SnapdUserInformation) user_information
        = snapd_client_login2_sync (client, "test@example.com", "secret",
                                    "0000", NULL, &error);
...
logout_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_logout_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_get_changes_sync (void)
{
    ...
    g_assert_cmpstr (snapd_change_get_summary (changes->pdata[0]), ==,
                     "SUMMARY");
    g_assert_cmpstr (snapd_change_get_status (changes->pdata[0]), ==, "Done");
    g_assert_true (snapd_change_get_ready (changes->pdata[0]));
    g_assert_true (
        date_matches (snapd_change_get_spawn_time (changes->pdata[0]), 2017, 1,
                      2, 11, 0, 0));
    g_assert_true (
        date_matches (snapd_change_get_ready_time (changes->pdata[0]), 2017, 1,
                      2, 11, 0, 30));
    ...
    g_assert_cmpstr (snapd_task_get_progress_label (tasks->pdata[0]), ==,
                     "LABEL");
    g_assert_cmpint (snapd_task_get_progress_done (tasks->pdata[0]), ==,
                     65535);
    g_assert_cmpint (snapd_task_get_progress_total (tasks->pdata[0]), ==,
                     65535);
    g_assert_true (date_matches (snapd_task_get_spawn_time (tasks->pdata[0]),
                                 2017, 1, 2, 11, 0, 0));
    g_assert_true (date_matches (snapd_task_get_ready_time (tasks->pdata[0]),
                                 2017, 1, 2, 11, 0, 10));
    ...
    g_assert_cmpstr (snapd_task_get_progress_label (tasks->pdata[1]), ==,
                     "LABEL");
    ...
    g_assert_true (date_matches (snapd_task_get_spawn_time (tasks->pdata[1]),
                                 2017, 1, 2, 11, 0, 10));
    g_assert_true (date_matches (snapd_task_get_ready_time (tasks->pdata[1]),
                                 2017, 1, 2, 11, 0, 30));

    g_assert_cmpstr (snapd_change_get_id (changes->pdata[1]), ==, "2");
    g_assert_cmpstr (snapd_change_get_kind (changes->pdata[1]), ==, "KIND");
    g_assert_cmpstr (snapd_change_get_summary (changes->pdata[1]), ==,
                     "SUMMARY");
    g_assert_cmpstr (snapd_change_get_status (changes->pdata[1]), ==, "Do");
    g_assert_false (snapd_change_get_ready (changes->pdata[1]));
    g_assert_true (
        date_matches (snapd_change_get_spawn_time (changes->pdata[1]), 2017, 1,
                      2, 11, 15, 0));
...
test_get_changes_async (void)
{
    ...
    snapd_client_get_changes_async (client, SNAPD_CHANGE_FILTER_ALL, NULL,
                                    NULL, get_changes_cb,
...
test_get_change_sync (void)
{
    ...
    g_assert_true (date_matches (snapd_change_get_spawn_time (change), 2017, 1,
                                 2, 11, 0, 0));
    g_assert_true (date_matches (snapd_change_get_ready_time (change), 2017, 1,
                                 2, 11, 0, 30));
...
test_list_sync (void)
{
    ...
    g_autoptr (GPtrArray) snaps
        = snapd_client_list_sync (client, NULL, &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
...
test_list_async (void)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    snapd_client_list_async (client, NULL, list_cb,
                             async_data_new (loop, snapd));
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_main_loop_run (loop);
}
...
test_get_snaps_inhibited (void)
{
    ...
    g_assert_cmpstr (snapd_snap_get_name (snaps->pdata[0]), ==, "snap1");
    g_assert_true (date_matches (snapd_snap_get_proceed_time (snaps->pdata[0]),
                                 2024, 3, 13, 15, 43, 32));
}
...
test_list_one_sync (void)
{
    ...
    g_assert_cmpint (snapd_snap_get_status (snap), ==,
                     SNAPD_SNAP_STATUS_ACTIVE);
...
test_get_snap_types (void)
{
    ...
    g_assert_cmpint (snapd_snap_get_snap_type (os_snap), ==,
                     SNAPD_SNAP_TYPE_OS);
    ...
    g_assert_cmpint (snapd_snap_get_snap_type (app_snap), ==,
                     SNAPD_SNAP_TYPE_APP);
}
...
test_get_snap_optional_fields (void)
{
    ...
    g_assert_cmpstr (snapd_snap_get_icon (snap), ==, "ICON");
    g_assert_cmpstr (snapd_snap_get_id (snap), ==, "ID");
    g_assert_true (date_matches (snapd_snap_get_install_date (snap), 2017, 1,
                                 2, 11, 23, 58));
...
test_get_snap_deprecated_fields (void)
{
    ...
    g_assert_nonnull (snap);
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    g_assert_cmpstr (snapd_snap_get_developer (snap), ==,
                     "PUBLISHER-USERNAME");
    G_GNUC_END_IGNORE_DEPRECATIONS
}
...
get_snap_conf_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    g_autoptr (GHashTable) conf = snapd_client_get_snap_conf_finish (
        SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_nonnull (conf);
...
setup_set_snap_conf (MockSnapd *snapd)
{
    ...
    g_hash_table_insert (key_values, "string-key",
    ...
    g_hash_table_insert (key_values, "number-key",
                         g_variant_new_double (1.25));
...
set_snap_conf_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r = snapd_client_set_snap_conf_finish (SNAPD_CLIENT (object),
                                                    result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_set_snap_conf_invalid (void)
{
    ...
    g_hash_table_insert (key_values, "string-value",
                         g_variant_new_string ("value"));
    gboolean r = snapd_client_set_snap_conf_sync (client, "invalid",
                                                  key_values, NULL, &error);
    g_assert_error (error, SNAPD_ERROR, SNAPD_ERROR_NOT_FOUND);
    g_assert_false (r);
}
...
test_get_apps_async (void)
{
    ...
    snapd_client_get_apps2_async (client, SNAPD_GET_APPS_FLAGS_NONE, NULL,
                                  NULL, get_apps_cb,
                                  async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
check_plug_no_attributes (SnapdPlug *plug)
{
    ...
    g_auto (GStrv) attribute_names
        = snapd_plug_get_attribute_names (plug, &length);
...
check_slot_no_attributes (SnapdSlot *slot)
{
    guint length;
    g_auto (GStrv) attribute_names
        = snapd_slot_get_attribute_names (slot, &length);
    g_assert_cmpint (length, ==, 0);
    g_assert_cmpint (g_strv_length (attribute_names), ==, 0);
}
...
static void
check_get_connections_result (GPtrArray *established,
                              GPtrArray *undesired,
                              GPtrArray *plugs,
                              GPtrArray *slots,
                              gboolean select_all)
{
    ...
    if (select_all) {
        ...
        plug_ref = snapd_connection_get_plug (connection);
        g_assert_cmpstr (snapd_plug_ref_get_snap (plug_ref), ==, "snap2");
        g_assert_cmpstr (snapd_plug_ref_get_plug (plug_ref), ==,
                         "undesired-plug");
        check_connection_no_slot_attributes (connection);
        check_connection_no_plug_attributes (connection);
        g_assert_true (snapd_connection_get_manual (connection));
...
get_connections_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    gboolean r = snapd_client_get_connections2_finish (
        SNAPD_CLIENT (object), result, &established, &undesired, &plugs,
        &slots, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_get_connections_async (void)
{
    ...
    snapd_client_get_connections2_async (
        client, SNAPD_GET_CONNECTIONS_FLAGS_NONE, NULL, NULL, NULL,
        get_connections_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
test_get_connections_filter_all (void)
{
    ...
    g_autoptr (GPtrArray) slots = NULL;
    gboolean result = snapd_client_get_connections2_sync (
        client, SNAPD_GET_CONNECTIONS_FLAGS_SELECT_ALL, NULL, NULL,
        &established, &undesired, &plugs, &slots, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
test_get_connections_filter_interface (void)
{
    ...
    g_autoptr (GPtrArray) slots = NULL;
    gboolean result = snapd_client_get_connections2_sync (
        client, SNAPD_GET_CONNECTIONS_FLAGS_NONE, NULL, "interface1", NULL,
        NULL, &plugs, &slots, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
test_get_connections_attributes (void)
{
    ...
    g_auto (GStrv) plug_attribute_names
        = snapd_connection_get_plug_attribute_names (connection,
                                                     &names_length);
    check_names_match (
    ...
    g_assert_true (g_variant_get_boolean (value));
    value
        = snapd_connection_get_plug_attribute (connection, "plug-number-key");
    g_assert_nonnull (value);
    g_assert_true (g_variant_is_of_type (value, G_VARIANT_TYPE_DOUBLE));
    g_assert_cmpfloat (g_variant_get_double (value), ==, 1.25);
    g_assert_false (
    ...
    g_auto (GStrv) slot_attribute_names
        = snapd_connection_get_slot_attribute_names (connection,
                                                     &names_length);
    check_names_match (
    ...
    value
        = snapd_connection_get_slot_attribute (connection, "slot-string-key");
    g_assert_true (g_variant_is_of_type (value, G_VARIANT_TYPE_STRING));
    g_assert_cmpstr (g_variant_get_string (value, NULL), ==, "value");
    g_assert_true (
    ...
    g_assert_true (g_variant_is_of_type (value, G_VARIANT_TYPE_BOOLEAN));
    value
        = snapd_connection_get_slot_attribute (connection, "slot-number-key");
    g_assert_nonnull (value);
    g_assert_true (g_variant_is_of_type (value, G_VARIANT_TYPE_DOUBLE));
    g_assert_cmpfloat (g_variant_get_double (value), ==, 1.25);
    g_assert_false (
...
test_get_interfaces_sync (void)
{
    ...
    g_autoptr (GPtrArray) slots = NULL;
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    gboolean result = snapd_client_get_interfaces_sync (client, &plugs, &slots,
                                                        NULL, &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
    g_assert_true (result);
...
get_interfaces_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    gboolean r = snapd_client_get_interfaces_finish (
        SNAPD_CLIENT (object), result, &plugs, &slots, &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
...
get_interfaces2_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    g_autoptr (GPtrArray) ifaces = snapd_client_get_interfaces2_finish (
        SNAPD_CLIENT (object), result, &error);
...
test_get_interfaces2_async (void)
{
    ...
    snapd_client_get_interfaces2_async (
        client, SNAPD_GET_INTERFACES_FLAGS_NONE, NULL, NULL,
        get_interfaces2_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
connect_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_true (mock_snapd_find_plug_connection (data->snapd, plug)
                   == slot);
...
test_connect_interface_async (void)
{
    ...
    snapd_client_connect_interface_async (client, "snap2", "plug", "snap1",
                                          "slot", NULL, NULL, NULL, connect_cb,
                                          async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
static void
connect_interface_progress_cb (SnapdClient *client,
                               SnapdChange *change,
                               gpointer deprecated,
                               gpointer user_data)
{
...
test_connect_interface_progress (void)
{
    ...
    gboolean result = snapd_client_connect_interface_sync (
        ...
        connect_interface_progress_cb, &connect_interface_progress_data, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_true (mock_snapd_find_plug_connection (snapd, plug) == slot);
...
test_disconnect_interface_async (void)
{
    ...
    snapd_client_disconnect_interface_async (
        client, "snap2", "plug", "snap1", "slot", NULL, NULL, NULL,
        disconnect_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
static void
disconnect_interface_progress_cb (SnapdClient *client,
                                  SnapdChange *change,
                                  gpointer deprecated,
                                  gpointer user_data)
{
...
test_find_channels (void)
{
    ...
    for (guint i = 0; i < channels->len; i++) {
        ...
        if (strcmp (snapd_channel_get_name (channel), "stable") == 0) {
            ...
            g_assert_cmpstr (snapd_channel_get_revision (channel), ==,
                             "REVISION");
            g_assert_cmpstr (snapd_channel_get_version (channel), ==,
                             "VERSION");
            g_assert_cmpstr (snapd_channel_get_epoch (channel), ==, "0");
            g_assert_cmpint (snapd_channel_get_confinement (channel), ==,
                             SNAPD_CONFINEMENT_STRICT);
        ...
        if (strcmp (snapd_channel_get_name (channel), "beta") == 0) {
            ...
            g_assert_true (
                date_matches (snapd_channel_get_released_at (channel), 2018, 1,
                              19, 13, 14, 15));
            matched_beta = TRUE;
        }
...
test_find_channels_match (void)
{
    ...
    g_autoptr (GPtrArray) snaps1
        = snapd_client_find_sync (client, SNAPD_FIND_FLAGS_MATCH_NAME,
                                  "stable-snap", NULL, NULL, &error);
    g_assert_no_error (error);
    ...
    /* Match branches */
    g_autoptr (GPtrArray) snaps4
        = snapd_client_find_sync (client, SNAPD_FIND_FLAGS_MATCH_NAME,
                                  "branch-snap", NULL, NULL, &error);
    g_assert_no_error (error);
...
find_cancel_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    g_autoptr (GPtrArray) snaps = snapd_client_find_finish (
        SNAPD_CLIENT (object), result, NULL, &error);
...
test_find_section_query (void)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    g_autoptr (GPtrArray) snaps = snapd_client_find_section_sync (
        client, SNAPD_FIND_FLAGS_NONE, "section", "carrot", NULL, NULL,
        &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
    g_assert_nonnull (snaps);
...
test_find_section_name (void)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    g_autoptr (GPtrArray) snaps = snapd_client_find_section_sync (
        client, SNAPD_FIND_FLAGS_MATCH_NAME, "section", "carrot1", NULL, NULL,
        &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
    g_assert_nonnull (snaps);
...
test_find_category_query (void)
{
    ...
    g_autoptr (GPtrArray) snaps = snapd_client_find_category_sync (
        client, SNAPD_FIND_FLAGS_NONE, "category", "carrot", NULL, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_nonnull (snaps);
    g_assert_cmpint (snaps->len, ==, 1);
...
test_install_sync_multiple (void)
{
    ...
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap1", NULL, NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    result = snapd_client_install2_sync (client, SNAPD_INSTALL_FLAGS_NONE,
                                         "snap2", NULL, NULL, NULL, NULL, NULL,
                                         &error);
    g_assert_no_error (error);
    g_assert_true (result);
    result = snapd_client_install2_sync (client, SNAPD_INSTALL_FLAGS_NONE,
                                         "snap3", NULL, NULL, NULL, NULL, NULL,
                                         &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
test_install_async (void)
{
    ...
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
    snapd_client_install2_async (client, SNAPD_INSTALL_FLAGS_NONE, "snap",
                                 NULL, NULL, NULL, NULL, NULL, install_cb,
                                 async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
test_install_async_multiple (void)
{
    ...
    AsyncData *data = async_data_new (loop, snapd);
    data->counter = 3;
    snapd_client_install2_async (client, SNAPD_INSTALL_FLAGS_NONE, "snap1",
                                 NULL, NULL, NULL, NULL, NULL,
                                 install_multiple_cb, data);
    snapd_client_install2_async (client, SNAPD_INSTALL_FLAGS_NONE, "snap2",
                                 NULL, NULL, NULL, NULL, NULL,
                                 install_multiple_cb, data);
    snapd_client_install2_async (client, SNAPD_INSTALL_FLAGS_NONE, "snap3",
                                 NULL, NULL, NULL, NULL, NULL,
                                 install_multiple_cb, data);
    g_main_loop_run (loop);
}
...
test_install_async_failure (void)
{
    ...
    snapd_client_install2_async (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap", NULL, NULL, NULL, NULL, NULL,
        install_failure_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
test_install_async_cancel (void)
{
    ...
    g_autoptr (GCancellable) cancellable = g_cancellable_new ();
    snapd_client_install2_async (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap", NULL, NULL, NULL, NULL,
        cancellable, install_cancel_cb, async_data_new (loop, snapd));
    g_idle_add (cancel_cb, cancellable);
    g_main_loop_run (loop);
}
...
static void
install_multiple_cancel_first_snap1_cb (GObject *object,
                                        GAsyncResult *result,
                                        gpointer user_data)
{
    AsyncData *data = user_data;
...
static void
install_multiple_cancel_first_cb (GObject *object,
                                  GAsyncResult *result,
                                  gpointer user_data)
{
    AsyncData *data = user_data;
...
test_install_async_multiple_cancel_first (void)
{
    ...
    data->counter = 3;
    snapd_client_install2_async (client, SNAPD_INSTALL_FLAGS_NONE, "snap1",
                                 NULL, NULL, NULL, NULL, cancellable,
...
static void
install_multiple_cancel_last_snap3_cb (GObject *object,
                                       GAsyncResult *result,
                                       gpointer user_data)
{
    AsyncData *data = user_data;
...
static void
install_multiple_cancel_last_cb (GObject *object,
                                 GAsyncResult *result,
                                 gpointer user_data)
{
    AsyncData *data = user_data;
...
test_install_progress (void)
{
    ...
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap", NULL, NULL,
        install_progress_cb, &install_progress_data, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
test_install_classic (void)
{
    ...
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_CLASSIC, "snap", NULL, NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_nonnull (mock_snapd_find_snap (snapd, "snap"));
...
test_install_devmode (void)
{
    ...
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_DEVMODE, "snap", NULL, NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_nonnull (mock_snapd_find_snap (snapd, "snap"));
...
test_install_dangerous (void)
{
    ...
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_DANGEROUS, "snap", NULL, NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_nonnull (mock_snapd_find_snap (snapd, "snap"));
...
test_install_jailmode (void)
{
    ...
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_JAILMODE, "snap", NULL, NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_nonnull (mock_snapd_find_snap (snapd, "snap"));
...
test_install_channel (void)
{
    ...
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap", "channel2", NULL, NULL, NULL,
        NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_nonnull (mock_snapd_find_snap (snapd, "snap"));
...
test_install_channel_not_available (void)
{
    ...
    gboolean result = snapd_client_install2_sync (
        client, SNAPD_INSTALL_FLAGS_NONE, "snap", "channel", NULL, NULL, NULL,
        NULL, &error);
...
install_stream_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r = snapd_client_install_stream_finish (SNAPD_CLIENT (object),
                                                     result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_install_stream_async (void)
{
    ...
    snapd_client_install_stream_async (
        client, SNAPD_INSTALL_FLAGS_NONE, stream, NULL, NULL, NULL,
        install_stream_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
static void
install_stream_progress_cb (SnapdClient *client,
                            SnapdChange *change,
                            gpointer deprecated,
                            gpointer user_data)
{
...
test_install_stream_dangerous (void)
{
    ...
    g_autoptr (GInputStream) stream
        = g_memory_input_stream_new_from_data ("SNAP", 4, NULL);
    gboolean result = snapd_client_install_stream_sync (
        client, SNAPD_INSTALL_FLAGS_DANGEROUS, stream, NULL, NULL, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_true (result);
    MockSnap *snap = mock_snapd_find_snap (snapd, "sideload");
...
test_install_stream_jailmode (void)
{
    ...
    g_autoptr (GInputStream) stream
        = g_memory_input_stream_new_from_data ("SNAP", 4, NULL);
    gboolean result = snapd_client_install_stream_sync (
        client, SNAPD_INSTALL_FLAGS_JAILMODE, stream, NULL, NULL, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_true (result);
    MockSnap *snap = mock_snapd_find_snap (snapd, "sideload");
...
test_try_sync (void)
{
    ...
    gboolean result = snapd_client_try_sync (client, "/path/to/snap", NULL,
                                             NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
try_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_try_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
static void
try_progress_cb (SnapdClient *client,
                 SnapdChange *change,
                 gpointer deprecated,
                 gpointer user_data)
{
...
test_refresh_sync (void)
{
    ...
    gboolean result = snapd_client_refresh_sync (client, "snap", NULL, NULL,
                                                 NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
}
...
test_refresh_async (void)
{
    ...
    snapd_client_refresh_async (client, "snap", NULL, NULL, NULL, NULL,
                                refresh_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
static void
refresh_progress_cb (SnapdClient *client,
                     SnapdChange *change,
                     gpointer deprecated,
                     gpointer user_data)
{
...
test_refresh_channel (void)
{
    ...
    gboolean result = snapd_client_refresh_sync (client, "snap", "channel2",
...
static void
refresh_all_progress_cb (SnapdClient *client,
                         SnapdChange *change,
                         gpointer deprecated,
                         gpointer user_data)
{
...
test_refresh_all_progress (void)
{
    ...
    g_auto (GStrv) snap_names = snapd_client_refresh_all_sync (
        client, refresh_all_progress_cb, &refresh_all_progress_data, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_cmpint (g_strv_length (snap_names), ==, 2);
    g_assert_cmpstr (snap_names[0], ==, "snap1");
...
test_remove_sync (void)
{
    ...
    gboolean result = snapd_client_remove2_sync (
        client, SNAPD_REMOVE_FLAGS_NONE, "snap", NULL, NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
...
static void
remove_progress_cb (SnapdClient *client,
                    SnapdChange *change,
                    gpointer deprecated,
                    gpointer user_data)
{
...
test_remove_purge (void)
{
    ...
    gboolean result = snapd_client_remove2_sync (
        client, SNAPD_REMOVE_FLAGS_PURGE, "snap", NULL, NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_null (mock_snapd_find_snap (snapd, "snap"));
...
enable_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_enable_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_false (
...
static void
enable_progress_cb (SnapdClient *client,
                    SnapdChange *change,
                    gpointer deprecated,
                    gpointer user_data)
{
...
test_enable_progress (void)
{
    ...
    gboolean result
        = snapd_client_enable_sync (client, "snap", enable_progress_cb,
                                    &enable_progress_data, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_false (
        mock_snap_get_disabled (mock_snapd_find_snap (snapd, "snap")));
...
static void
disable_progress_cb (SnapdClient *client,
                     SnapdChange *change,
                     gpointer deprecated,
                     gpointer user_data)
{
...
test_switch_sync (void)
{
    ...
    gboolean result = snapd_client_switch_sync (client, "snap", "beta", NULL,
                                                NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_cmpstr (
...
switch_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_switch_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_cmpstr (mock_snap_get_tracking_channel (
                         mock_snapd_find_snap (data->snapd, "snap")),
                     ==, "beta");
...
test_switch_async (void)
{
    ...
    snapd_client_switch_async (client, "snap", "beta", NULL, NULL, NULL,
                               switch_cb, async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
static void
switch_progress_cb (SnapdClient *client,
                    SnapdChange *change,
                    gpointer deprecated,
                    gpointer user_data)
{
...
check_buy_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r = snapd_client_check_buy_finish (SNAPD_CLIENT (object), result,
                                                &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_create_user_sync (void)
{
    ...
    g_autoptr (SnapdUserInformation) info = snapd_client_create_user_sync (
        client, "user@example.com", SNAPD_CREATE_USER_FLAGS_NONE, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_nonnull (info);
...
test_create_user_sudo (void)
{
    ...
    g_assert_null (mock_snapd_find_account_by_username (snapd, "user"));
    g_autoptr (SnapdUserInformation) info = snapd_client_create_user_sync (
        client, "user@example.com", SNAPD_CREATE_USER_FLAGS_SUDO, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_nonnull (info);
    MockAccount *account = mock_snapd_find_account_by_username (snapd, "user");
...
test_create_user_known (void)
{
    ...
    g_assert_null (mock_snapd_find_account_by_username (snapd, "user"));
    g_autoptr (SnapdUserInformation) info = snapd_client_create_user_sync (
        client, "user@example.com", SNAPD_CREATE_USER_FLAGS_KNOWN, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_nonnull (info);
    MockAccount *account = mock_snapd_find_account_by_username (snapd, "user");
...
get_users_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    g_autoptr (GPtrArray) users_info = snapd_client_get_users_finish (
        SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_nonnull (users_info);
    g_assert_cmpint (users_info->len, ==, 2);
...
test_get_sections_sync (void)
{
    ...
    g_auto (GStrv) sections
        = snapd_client_get_sections_sync (client, NULL, &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
...
get_categories_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    g_autoptr (GPtrArray) categories = snapd_client_get_categories_finish (
        SNAPD_CLIENT (object), result, &error);
    G_GNUC_END_IGNORE_DEPRECATIONS
    g_assert_no_error (error);
    g_assert_nonnull (categories);
...
compare_alias_name (gconstpointer a, gconstpointer b)
{
    ...
    return strcmp (snapd_alias_get_name (alias_a),
                   snapd_alias_get_name (alias_b));
}
...
test_aliases_alias_sync (void)
{
    ...
    gboolean result = snapd_client_alias_sync (client, "snap", "app", "foo",
                                               NULL, NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
alias_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_alias_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_nonnull (mock_app_find_alias (a, "foo"));
...
test_aliases_unalias_sync (void)
{
    ...
    gboolean result = snapd_client_unalias_sync (client, "snap", "foo", NULL,
                                                 NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_null (mock_app_find_alias (a, "foo"));
...
test_aliases_unalias_no_snap_sync (void)
{
    ...
    gboolean result = snapd_client_unalias_sync (client, NULL, "foo", NULL,
                                                 NULL, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    g_assert_null (mock_app_find_alias (a, "foo"));
...
prefer_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r
        = snapd_client_prefer_finish (SNAPD_CLIENT (object), result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_true (mock_snap_get_preferred (s));
...
test_run_snapctl_sync (void)
{
    ...
    gboolean result = snapd_client_run_snapctl2_sync (
        client, "ABC", args, &stdout_output, &stderr_output, &exit_code, NULL,
        &error);
    g_assert_no_error (error);
    g_assert_true (result);
...
snapctl_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    int exit_code = 0;
    g_autoptr (GError) error = NULL;
    gboolean r = snapd_client_run_snapctl2_finish (
        SNAPD_CLIENT (object), result, &stdout_output, &stderr_output,
        &exit_code, &error);
    g_assert_no_error (error);
    g_assert_true (r);
    g_assert_cmpstr (stdout_output, ==, "STDOUT:ABC:arg1:arg2");
...
test_run_snapctl_unsuccessful (void)
{
    ...
    int exit_code = 0;
    gboolean result = snapd_client_run_snapctl2_sync (
        client, "return-error", args, &stdout_output, &stderr_output,
...
test_download_sync (void)
{
    ...
    g_autoptr (GBytes) snap_data = snapd_client_download_sync (
        client, "test", NULL, NULL, NULL, &error);
    g_assert_no_error (error);
...
test_themes_check_sync (void)
{
    ...
    char *sound_themes[]
        = { "soundtheme1", "soundtheme2", "soundtheme3", NULL };
    ...
    gboolean result = snapd_client_check_themes_sync (
        client, gtk_themes, icon_themes, sound_themes, &gtk_status,
        &icon_status, &sound_status, NULL, &error);
    g_assert_no_error (error);
    g_assert_true (result);
    ...
    g_assert_cmpint (
        GPOINTER_TO_INT (g_hash_table_lookup (gtk_status, "gtktheme1")), ==,
        SNAPD_THEME_STATUS_INSTALLED);
    g_assert_cmpint (
        GPOINTER_TO_INT (g_hash_table_lookup (gtk_status, "gtktheme2")), ==,
        SNAPD_THEME_STATUS_AVAILABLE);
    g_assert_cmpint (
        GPOINTER_TO_INT (g_hash_table_lookup (gtk_status, "gtktheme3")), ==,
...
check_themes_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GHashTable) icon_status = NULL;
    g_autoptr (GHashTable) sound_status = NULL;
    gboolean res = snapd_client_check_themes_finish (
        SNAPD_CLIENT (object), result, &gtk_status, &icon_status,
        &sound_status, &error);
    g_assert_no_error (error);
    g_assert_true (res);
    g_assert_nonnull (gtk_status);
...
test_themes_check_async (void)
{
    ...
    char *sound_themes[]
        = { "soundtheme1", "soundtheme2", "soundtheme3", NULL };
    snapd_client_check_themes_async (client, gtk_themes, icon_themes,
                                     sound_themes, NULL, check_themes_cb,
                                     async_data_new (loop, snapd));
    g_main_loop_run (loop);
}
...
install_themes_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean res = snapd_client_install_themes_finish (SNAPD_CLIENT (object),
                                                       result, &error);
    g_assert_no_error (error);
    g_assert_true (res);
...
follow_logs_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    g_autoptr (GError) error = NULL;
    gboolean r = snapd_client_follow_logs_finish (SNAPD_CLIENT (object),
                                                  result, &error);
    g_assert_no_error (error);
    g_assert_true (r);
...
test_get_changes_data (void)
{
    ...
    GStrv refresh_forced
        = snapd_autorefresh_change_data_get_refresh_forced (data);
...
void
test_notices_events_cb (SnapdClient *source_object,
                        GAsyncResult *result,
                        gpointer user_data)
{
    AsyncData *data = user_data;
    ...
    g_autoptr (GPtrArray) notices
        = snapd_client_get_notices_finish (source_object, result, &error);
    g_assert_no_error (error);
    ...
    g_assert_true (
        g_date_time_equal (snapd_notice_get_first_occurred (notice1), date1));
    g_assert_true (
        g_date_time_equal (snapd_notice_get_last_occurred (notice1), date2));
    g_assert_true (
        g_date_time_equal (snapd_notice_get_last_repeated (notice1), date3));
    ...
    g_assert_true (
        g_date_time_equal (snapd_notice_get_first_occurred (notice2), date4));
    g_assert_true (
        g_date_time_equal (snapd_notice_get_last_occurred (notice2), date5));
    g_assert_true (
        g_date_time_equal (snapd_notice_get_last_repeated (notice2), date4));
    ...
    if (data->counter == 0) {
        ...
        g_autoptr (GHashTable) parameters = g_uri_parse_params (
            mock_snapd_get_notices_parameters (data->snapd), -1, "&",
        ...
        g_assert_cmpstr (g_hash_table_lookup (parameters, "keys"), ==,
                         "key1,key2");
        ...
        g_assert_cmpstr (g_hash_table_lookup (parameters, "timeout"), ==,
                         "20000us");
#endif
        data->counter++;
        snapd_client_get_notices_async (
            source_object, NULL, 0, NULL,
            (GAsyncReadyCallback)test_notices_events_cb, data);
    } else {
...
test_notices_events (void)
{
    ...
    snapd_client_get_notices_with_filters_async (
        client, "an_user_id", "id1, id2, an_utf8_íd", "type1,type2",
        "key1,key2", date5, 20000, NULL,
        (GAsyncReadyCallback)test_notices_events_cb, data);
    g_main_loop_run (loop);
}

void
test_notices_minimal_data_events_cb (SnapdClient *source_object,
                                     GAsyncResult *result,
                                     gpointer user_data)
{
    AsyncData *data = user_data;
    ...
    if (data->counter == 0) {
        ...
        snapd_client_get_notices_async (
            source_object, date5, 0, NULL,
            (GAsyncReadyCallback)test_notices_minimal_data_events_cb, data);
    } else {
#if GLIB_CHECK_VERSION(2, 66, 0)
        g_autoptr (GHashTable) parameters = g_uri_parse_params (
            mock_snapd_get_notices_parameters (data->snapd), -1, "&",
...
test_notice_comparison (void)
{
    ...
    g_autoptr (GDateTime) date0
        = g_date_time_new (timezone, 2023, 5, 3, 22, 20, 7);
    g_autoptr (GDateTime) date1
    ...
    g_assert_true (snapd_notice_compare_last_occurred (notice1, notice3)
                   == -1);
    g_assert_true (snapd_notice_compare_last_occurred (notice1, notice4) == 1);
    g_assert_true (snapd_notice_compare_last_occurred (notice1, notice5)
                   == -1);
}
...
test_error_get_change (void)
{
    ...
    g_autoptr (SnapdChange) change2 = snapd_client_get_change_sync (
        client, "aninexistentID", NULL, &error);
...
main (int argc, char **argv)
{
    ...
    g_test_add_func ("/notices/test_notice_comparison",
                     test_notice_comparison);
