...
void
QSnapdMarkdownParser::setPreserveWhitespace (bool preserveWhitespace) const
{
    Q_D (const QSnapdMarkdownParser);
    snapd_markdown_parser_set_preserve_whitespace (d->parser,
                                                   preserveWhitespace);
}

bool
QSnapdMarkdownParser::preserveWhitespace () const
{
    Q_D (const QSnapdMarkdownParser);
...
QSnapdMarkdownParser::parse (const QString &text) const
{
    ...
    for (uint i = 0; i < nodes->len; i++) {
        SnapdMarkdownNode *node
            = (SnapdMarkdownNode *)g_ptr_array_index (nodes, i);
