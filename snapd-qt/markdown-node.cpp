...
QSnapdMarkdownNode::NodeType
QSnapdMarkdownNode::type () const
{
    switch (snapd_markdown_node_get_node_type (
        SNAPD_MARKDOWN_NODE (wrapped_object))) {
    default:
...
QString
QSnapdMarkdownNode::text () const
{
    return snapd_markdown_node_get_text (SNAPD_MARKDOWN_NODE (wrapped_object));
}

int
QSnapdMarkdownNode::childCount () const
{
    GPtrArray *children;

    children = snapd_markdown_node_get_children (
        SNAPD_MARKDOWN_NODE (wrapped_object));
    return children != NULL ? children->len : 0;
}

QSnapdMarkdownNode *
QSnapdMarkdownNode::child (int n) const
{
    GPtrArray *children;
