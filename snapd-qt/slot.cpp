...
bool
QSnapdSlot::hasAttribute (const QString &name) const
{
    return snapd_slot_has_attribute (SNAPD_SLOT (wrapped_object),
                                     name.toStdString ().c_str ());
}

QVariant
QSnapdSlot::attribute (const QString &name) const
{
    GVariant *value = snapd_slot_get_attribute (SNAPD_SLOT (wrapped_object),
                                                name.toStdString ().c_str ());
    return gvariant_to_qvariant (value);
}

QString
QSnapdSlot::label () const
{
    return snapd_slot_get_label (SNAPD_SLOT (wrapped_object));
}

int
QSnapdSlot::connectionCount () const
{
...
QSnapdConnection *
QSnapdSlot::connection (int n) const
{
    GPtrArray *connections;
...
int
QSnapdSlot::connectedPlugCount () const
{
    GPtrArray *connections;
...
QSnapdPlugRef *
QSnapdSlot::connectedPlug (int n) const
{
    GPtrArray *connections;
