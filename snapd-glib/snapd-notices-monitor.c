...
static void
monitor_cb (SnapdClient *source, GAsyncResult *res, SnapdNoticesMonitor *self)
{
    ...
    if (notices != NULL) {
        ...
        for (int i = 0; i < notices->len; i++) {
            ...
            if (self->last_notice == NULL
                || snapd_notice_compare_last_occurred (self->last_notice,
                                                       notice)
                       <= 0) {
...
begin_monitor (SnapdNoticesMonitor *self)
{
    ...
    g_autoptr (GDateTime) last_date_time
        = self->last_notice == NULL
              ? NULL
              : (GDateTime *)snapd_notice_get_last_occurred (
                    self->last_notice);
 ...
 */
gboolean
snapd_notices_monitor_start (SnapdNoticesMonitor *self, GError **error)
{
...
static void
snapd_notices_monitor_set_property (GObject *object,
                                    guint prop_id,
                                    const GValue *value,
                                    GParamSpec *pspec)
{
...
snapd_notices_monitor_class_init (SnapdNoticesMonitorClass *klass)
{
    ...
    g_signal_new ("notice-event", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
                  0, NULL, NULL, NULL, G_TYPE_NONE, 2, SNAPD_TYPE_NOTICE,
                  G_TYPE_BOOLEAN);
    g_signal_new ("error-event", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
...
snapd_notices_monitor_new (void)
{
    ...
    SnapdNoticesMonitor *self = g_object_new (
        snapd_notices_monitor_get_type (), "client", client, NULL);
    return self;
}
