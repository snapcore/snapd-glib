 ...
 * snapd. Useful when combined with
 * #snapd_client_notices_set_since_nanoseconds, and used internally by
 * snapd_client_notices_set_after_notice, to ensure the maximum possible
 * precission when dealing with timestamps.
 *
 * Returns: a gint32 with the nanosecond value between 0 and 999999999, or -1
 * if no nanosecond value was set.
 *
 * Since: 1.66
 */
const gint32
 ...
 * available) of both notices, and returns whether the first one is before,
 * same or after the second one.
 *
 * Returns: -1 if the first one is before the second one; 0 if both are the
 * same time instant, and 1 if the first one is after the second one.
 *
 * Since: 1.66
 */

const gint
snapd_notice_compare_last_occurred (SnapdNotice *self,
                                    SnapdNotice *notice_to_compare)
{
    ...
    if (notice_nano == -1)
        notice_nano
            = 1000
              * g_date_time_get_microsecond (notice_to_compare->last_occurred);
...
static void
snapd_notice_set_property (GObject *object,
                           guint prop_id,
                           const GValue *value,
                           GParamSpec *pspec)
{
    ...
    case PROP_FIRST_OCCURRED:
        ...
        self->first_occurred
            = tmpdate == NULL ? NULL : g_date_time_ref (tmpdate);
        break;
    case PROP_LAST_OCCURRED:
        ...
        if (g_value_get_boxed (value) != NULL) {
            ...
            self->last_occurred_internal = g_date_time_add (
                self->last_occurred,
...
static void
snapd_notice_get_property (GObject *object,
                           guint prop_id,
                           GValue *value,
                           GParamSpec *pspec)
{
...
snapd_notice_class_init (SnapdNoticeClass *klass)
{
    ...
    g_object_class_install_property (
        ...
        g_param_spec_int (
            "last-occurred-nanoseconds", "last-occurred-nanoseconds",
            "Time this notice last ocurred, in string format and with "
            "nanosecond accuracy",
            -1, 999999999, -1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property (
    ...
    g_object_class_install_property (
        ...
        g_param_spec_int64 ("repeat-after", "repeat-after",
                            "Time (in ms) after one of these was last "
                            "repeated should we allow it to repeat",
                            G_MININT64, G_MAXINT64, 0,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property (
        gobject_class, PROP_EXPIRE_AFTER,
        g_param_spec_int64 ("expire-after", "expire-after",
                            "Time (in ms) since one of these last occurred "
                            "until we should drop the notice",
                            G_MININT64, G_MAXINT64, 0,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
