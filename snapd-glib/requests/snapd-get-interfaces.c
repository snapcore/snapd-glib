...
G_DEFINE_TYPE (SnapdGetInterfaces,
               snapd_get_interfaces,
               snapd_request_get_type ())

SnapdGetInterfaces *
_snapd_get_interfaces_new (gchar **names,
...
void
_snapd_get_interfaces_set_include_docs (SnapdGetInterfaces *self,
                                        gboolean include_docs)
{
...
void
_snapd_get_interfaces_set_include_plugs (SnapdGetInterfaces *self,
                                         gboolean include_plugs)
{
...
void
_snapd_get_interfaces_set_include_slots (SnapdGetInterfaces *self,
                                         gboolean include_slots)
{
...
void
_snapd_get_interfaces_set_only_connected (SnapdGetInterfaces *self,
                                          gboolean only_connected)
{
...
generate_get_interfaces_request (SnapdRequest *request, GBytes **body)
{
    ...
    if (self->names != NULL) {
        ...
        g_ptr_array_add (query_attributes,
                         g_strdup_printf ("names=%s", names_list));
    }
    ...
    g_ptr_array_add (query_attributes,
                     g_strdup_printf ("select=%s", self->only_connected
                                                       ? "connected"
                                                       : "all"));
                               ...
                               GError **error)
{
    ...
    if (response == NULL)
        return FALSE;
    g_autoptr (JsonArray) result
        = _snapd_json_get_sync_result_a (response, error);
    if (result == NULL)
        return FALSE;
