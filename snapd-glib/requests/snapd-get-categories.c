...
G_DEFINE_TYPE (SnapdGetCategories,
               snapd_get_categories,
               snapd_request_get_type ())

SnapdGetCategories *
_snapd_get_categories_new (GCancellable *cancellable,
...
static gboolean
parse_get_categories_response (SnapdRequest *request,
                               guint status_code,
                               ...
                               GError **error)
{
    ...
    for (guint i = 0; i < json_array_get_length (result); i++) {
        ...
        details = g_object_new (snapd_category_details_get_type (), "name",
                                name, NULL);
