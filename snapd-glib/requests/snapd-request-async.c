...
G_DEFINE_TYPE_WITH_PRIVATE (SnapdRequestAsync,
                            snapd_request_async,
...
gboolean
_snapd_request_async_parse_result (SnapdRequestAsync *self,
                                   JsonNode *result,
                                   GError **error)
{
    if (SNAPD_REQUEST_ASYNC_GET_CLASS (self)->parse_result == NULL)
        return TRUE;
    return SNAPD_REQUEST_ASYNC_GET_CLASS (self)->parse_result (self, result,
                                                               error);
}

static gboolean
parse_async_response (SnapdRequest *self,
                      guint status_code,
                      ...
                      GError **error)
{
    ...
    SnapdRequestAsyncPrivate *priv
        = snapd_request_async_get_instance_private (r);

    g_autoptr (JsonObject) response = _snapd_json_parse_response (
        content_type, body, maintenance, NULL, error);
    if (response == NULL)
        return FALSE;
    g_autofree gchar *change_id
        = _snapd_json_get_async_result (response, error);
    if (change_id == NULL)
        return FALSE;
...
{
    return g_strcmp0 (snapd_task_get_id (task1), snapd_task_get_id (task2))
               == 0
           && g_strcmp0 (snapd_task_get_kind (task1),
                         snapd_task_get_kind (task2))
                  == 0
           && g_strcmp0 (snapd_task_get_summary (task1),
                         snapd_task_get_summary (task2))
                  == 0
           && g_strcmp0 (snapd_task_get_status (task1),
                         snapd_task_get_status (task2))
                  == 0
           && g_strcmp0 (snapd_task_get_progress_label (task1),
                         snapd_task_get_progress_label (task2))
                  == 0
...
void
_snapd_request_async_report_progress (SnapdRequestAsync *self,
                                      SnapdClient *client,
                                      SnapdChange *change)
{
    SnapdRequestAsyncPrivate *priv
...
static void
snapd_request_async_set_property (GObject *object,
                                  guint prop_id,
                                  const GValue *value,
                                  GParamSpec *pspec)
{
