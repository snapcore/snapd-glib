...
generate_get_snap_request (SnapdRequest *request, GBytes **body)
{
    ...
    if (self->since_date_time != NULL) {
        ...
        if (self->since_date_time_nanoseconds == -1) {
            date_time
                = g_date_time_format (self->since_date_time, "%FT%T.%f%:z");
        } else {
            if (self->since_date_time_nanoseconds == 0) {
                date_time
                    = g_date_time_format (self->since_date_time, "%FT%T%:z");
            } else {
                // create a temporal date-time string with a %09d between
                // seconds and timezone
                g_autofree gchar *date_time_tmp = g_date_time_format (
                    self->since_date_time, "%FT%T.%%09d%:z");
                // Now use that %s to include the nanoseconds
                date_time = g_strdup_printf (
                    date_time_tmp, self->since_date_time_nanoseconds);
            }
        }
