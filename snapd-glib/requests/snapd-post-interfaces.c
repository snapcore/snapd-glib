...
G_DEFINE_TYPE (SnapdPostInterfaces,
               snapd_post_interfaces,
...
_snapd_post_interfaces_new (const gchar *action,
                            ...
                            gpointer user_data)
{
    SnapdPostInterfaces *self = SNAPD_POST_INTERFACES (g_object_new (
        snapd_post_interfaces_get_type (), "cancellable", cancellable,
...
generate_post_interfaces_request (SnapdRequest *request, GBytes **body)
{
    ...
    SoupMessage *message
        = soup_message_new ("POST", "http://snapd/v2/interfaces");
