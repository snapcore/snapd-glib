...
G_DEFINE_TYPE (SnapdPostSnapStream,
               snapd_post_snap_stream,
...
void
_snapd_post_snap_stream_set_classic (SnapdPostSnapStream *self,
                                     gboolean classic)
{
...
void
_snapd_post_snap_stream_set_dangerous (SnapdPostSnapStream *self,
                                       gboolean dangerous)
{
...
void
_snapd_post_snap_stream_set_devmode (SnapdPostSnapStream *self,
                                     gboolean devmode)
{
...
void
_snapd_post_snap_stream_set_jailmode (SnapdPostSnapStream *self,
                                      gboolean jailmode)
{
...
void
_snapd_post_snap_stream_append_data (SnapdPostSnapStream *self,
                                     const guint8 *data,
                                     guint len)
{
...
static void
append_multipart_value (SoupMultipart *multipart,
                        const gchar *name,
                        const gchar *value)
{
    ...
    soup_message_headers_set_content_disposition (headers, "form-data",
                                                  params);
...
generate_post_snap_stream_request (SnapdRequest *request, GBytes **body)
{
    ...
    soup_message_headers_set_content_disposition (request_headers, "form-data",
                                                  params);
    soup_message_headers_set_content_type (request_headers,
                                           "application/vnd.snap", NULL);
#if SOUP_CHECK_VERSION(2, 99, 2)
    g_autoptr (GBytes) part_buffer = g_bytes_new_static (
        self->snap_contents->data, self->snap_contents->len);
#else
    g_autoptr (SoupBuffer) part_buffer
        = soup_buffer_new (SOUP_MEMORY_STATIC, self->snap_contents->data,
                           self->snap_contents->len);
#endif
