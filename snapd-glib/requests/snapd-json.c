...
void
_snapd_json_set_body (SoupMessage *message,
                      JsonBuilder *builder,
                      GBytes **body)
{
...
gboolean
_snapd_json_get_bool (JsonObject *object,
                      const gchar *name,
...
gint64
_snapd_json_get_int (JsonObject *object,
                     const gchar *name,
...
const gchar *
_snapd_json_get_string (JsonObject *object,
                        const gchar *name,
...
gboolean
_snapd_json_parse_time_span (const gchar *time_span_string,
                             GTimeSpan *time_span)
{
    /* Examples:
          3y
...
GDateTime *
_snapd_json_get_date_time (JsonObject *object,
                           const gchar *name,
                           gint32 *nanoseconds)
{
    ...
    if (tokens[1] != NULL) {
        ...
        while (*timezone_start != '\0'
               && !is_timezone_prefix (*timezone_start))
            timezone_start++;
        if (*timezone_start != '\0') {
...
parse_error_response (JsonObject *root, JsonNode **error_value, GError **error)
{
    ...
    const gchar *kind = result != NULL
                            ? _snapd_json_get_string (result, "kind", NULL)
                            : NULL;
    const gchar *message
        = result != NULL ? _snapd_json_get_string (result, "message", NULL)
                         : NULL;
    ...
    if (g_strcmp0 (kind, "login-required") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_AUTH_DATA_REQUIRED, message);
        ...
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_TWO_FACTOR_REQUIRED, message);
    else if (g_strcmp0 (kind, "two-factor-failed") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_TWO_FACTOR_INVALID, message);
    else if (g_strcmp0 (kind, "terms-not-accepted") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_TERMS_NOT_ACCEPTED, message);
        ...
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NOT_INSTALLED,
                             message);
    else if (g_strcmp0 (kind, "snap-not-found") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NOT_FOUND,
                             message);
    else if (g_strcmp0 (kind, "snap-local") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NOT_IN_STORE,
                             message);
    else if (g_strcmp0 (kind, "snap-no-update-available") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_NO_UPDATE_AVAILABLE, message);
    else if (g_strcmp0 (kind, "password-policy") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_PASSWORD_POLICY_ERROR, message);
    else if (g_strcmp0 (kind, "snap-needs-devmode") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NEEDS_DEVMODE,
                             message);
    else if (g_strcmp0 (kind, "snap-needs-classic") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NEEDS_CLASSIC,
                             message);
    else if (g_strcmp0 (kind, "snap-needs-classic-system") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_NEEDS_CLASSIC_SYSTEM, message);
    else if (g_strcmp0 (kind, "bad-query") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_BAD_QUERY,
                             message);
        ...
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NOT_CLASSIC,
                             message);
    else if (g_strcmp0 (kind, "snap-revision-not-available") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_REVISION_NOT_AVAILABLE, message);
    else if (g_strcmp0 (kind, "snap-channel-not-available") == 0)
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_CHANNEL_NOT_AVAILABLE, message);
    else if (g_strcmp0 (kind, "snap-not-a-snap") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_NOT_A_SNAP,
                             message);
    else if (g_strcmp0 (kind, "dns-failure") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_DNS_FAILURE,
                             message);
        ...
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_UNSUCCESSFUL,
                             message);
    else if (g_strcmp0 (kind, "app-not-found") == 0)
        g_set_error_literal (error, SNAPD_ERROR, SNAPD_ERROR_APP_NOT_FOUND,
                             message);
        ...
        g_set_error_literal (error, SNAPD_ERROR,
                             SNAPD_ERROR_INTERFACES_UNCHANGED, message);
    else {
        switch (status_code) {
        case SOUP_STATUS_BAD_REQUEST:
            g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_BAD_REQUEST, "%s: %s",
                         kind, message);
            break;
        case SOUP_STATUS_UNAUTHORIZED:
            g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_AUTH_DATA_REQUIRED,
                         "%s: %s", kind, message);
            break;
        case SOUP_STATUS_FORBIDDEN:
            g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_PERMISSION_DENIED,
                         "%s: %s", kind, message);
            break;
        case SOUP_STATUS_NOT_FOUND:
            g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_NOT_FOUND, "%s: %s",
                         kind, message);
            break;
...
JsonObject *
_snapd_json_parse_response (const gchar *content_type,
                            GBytes *body,
                            ...
                            GError **error)
{
    ...
    if (g_strcmp0 (content_type, "application/json") != 0) {
        g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_BAD_RESPONSE,
                     "snapd returned unexpected content type %s",
                     content_type);
        return NULL;
    }
...
_snapd_json_get_sync_result (JsonObject *response, GError **error)
{
    ...
    if (strcmp (type, "sync") != 0) {
        g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                     "Unexpected response '%s' returned for sync request",
                     type);
        return NULL;
    }
...
_snapd_json_get_async_result (JsonObject *response, GError **error)
{
    ...
    if (strcmp (type, "async") != 0) {
        g_set_error (error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                     "Unexpected response '%s' returned for async request",
                     type);
        return NULL;
    }
...
static void
add_notice_to_list (JsonArray *array,
                    guint index,
                    JsonNode *element,
                    void *data)
{
    ...
    if (json_object_has_member (object, "last-data")) {
        ...
        while (json_object_iter_next (&iter, &member_name, &member_node))
            g_hash_table_insert (
                last_data, g_strdup ((gchar *)member_name),
                g_strdup ((gchar *)json_node_get_string (member_node)));
    }
    g_autofree const gchar *user_id
        = _snapd_json_get_int_as_string (object, "user-id");
    g_autoptr (SnapdNotice) notice = g_object_new (
        ...
        "last-repeated", last_repeated, "occurrences",
        _snapd_json_get_int (object, "occurrences", -1), "expire-after",
        expire_after, "repeat-after", repeat_after, "last-data", last_data,
        NULL);
...
_snapd_json_parse_notice (JsonNode *node, GError **error)
{
    ...
    GPtrArray *retlist = g_ptr_array_new_full (json_array_get_length (notices),
                                               g_object_unref);
...
_snapd_json_parse_change (JsonNode *node, GError **error)
{
    ...
    for (guint i = 0; i < json_array_get_length (array); i++) {
        ...
        g_autoptr (SnapdTask) t = g_object_new (
            ...
            "spawn-time", spawn_time, "ready-time", ready_time, NULL);
    ...
    if (g_str_equal (kind, "auto-refresh") && (autorefresh_data != NULL)) {
        ...
        g_auto (GStrv) snap_names
            = create_str_array_from_jsonarray (snap_names_json);
        ...
        data = g_object_new (SNAPD_TYPE_AUTOREFRESH_CHANGE_DATA, "snap-names",
                             snap_names, "refresh-forced", refresh_forced,
                             NULL);
    }
    ...
    return g_object_new (
        ...
        "kind", _snapd_json_get_string (object, "kind", NULL), "summary",
        _snapd_json_get_string (object, "summary", NULL), "status",
        _snapd_json_get_string (object, "status", NULL), "tasks", tasks,
        "ready", _snapd_json_get_bool (object, "ready", FALSE), "spawn-time",
...
_snapd_json_parse_system_information (JsonNode *node, GError **error)
{
    ...
    if (sandbox_features != NULL) {
        ...
        while (json_object_iter_next (&iter, &name, &features_node)) {
            ...
            for (guint i = 0; i < json_array_get_length (features_array);
                 i++) {
    ...
    return g_object_new (
        ...
        refresh_hold, "refresh-last", refresh_last, "refresh-next",
        refresh_next, "refresh-schedule",
        _snapd_json_get_string (refresh, "schedule", NULL), "refresh-timer",
        _snapd_json_get_string (refresh, "timer", NULL), NULL);
}
...
_snapd_json_parse_snap (JsonNode *node, GError **error)
{
    ...
    SnapdConfinement confinement = parse_confinement (
        _snapd_json_get_string (object, "confinement", ""));

    const gchar *snap_type_string
        = _snapd_json_get_string (object, "type", "");
    ...
    g_autoptr (JsonArray) categories
        = _snapd_json_get_array (object, "categories");
    ...
    for (guint i = 0; i < json_array_get_length (categories); i++) {
        ...
        g_autoptr (SnapdCategory) category = g_object_new (
            SNAPD_TYPE_CATEGORY, "featured",
    ...
    if (channels != NULL) {
        ...
        while (json_object_iter_next (&iter, &name, &channel_node)) {
            ...
            SnapdConfinement confinement = parse_confinement (
                _snapd_json_get_string (c, "confinement", ""));
            ...
            g_autoptr (SnapdChannel) channel = g_object_new (
                ...
                _snapd_json_get_string (c, "revision", NULL), "size",
                _snapd_json_get_int (c, "size", 0), "version",
                _snapd_json_get_string (c, "version", NULL), NULL);
            g_ptr_array_add (channels_array, g_steal_pointer (&channel));
        }
    }

    g_autoptr (JsonArray) common_ids
        = _snapd_json_get_array (object, "common-ids");
    ...
    for (guint i = 0; i < json_array_get_length (common_ids); i++) {
        ...
        g_ptr_array_add (common_ids_array,
                         (gpointer)json_node_get_string (node));
    }
    ...
    g_autoptr (GDateTime) hold
        = _snapd_json_get_date_time (object, "hold", NULL);
    ...
    /* The tracks field was originally incorrectly named, fixed in snapd
     * 61ad9ed (2.29.5) */
    ...
    if (publisher != NULL) {
        ...
        publisher_username = _snapd_json_get_string (publisher, "username",
                                                     publisher_username);
    ...
    if (refresh_inhibit != NULL)
        proceed_time = _snapd_json_get_date_time (refresh_inhibit,
                                                  "proceed-time", NULL);
    ...
    return g_object_new (
        ...
        _snapd_json_get_string (object, "id", NULL), "install-date",
        install_date, "installed-size",
        _snapd_json_get_int (object, "installed-size", 0), "jailmode",
        _snapd_json_get_bool (object, "jailmode", FALSE), "license",
        ...
        "publisher-validation", publisher_validation, "revision",
        ...
        "summary", _snapd_json_get_string (object, "summary", NULL), "title",
...
SnapdAlias *
_snapd_json_parse_alias (JsonNode *node,
                         ...
                         GError **error)
{
    ...
    const gchar *status_string
        = _snapd_json_get_string (object, "status", NULL);
    ...
    return g_object_new (
        SNAPD_TYPE_ALIAS, "snap", snap_name, "app-auto",
        _snapd_json_get_string (object, "auto", NULL), "app-manual",
        _snapd_json_get_string (object, "manual", NULL), "command",
        _snapd_json_get_string (object, "command", NULL), "name", name,
        "status", status, NULL);
}
...
_snapd_json_parse_user_information (JsonNode *node, GError **error)
{
    ...
    g_autoptr (JsonArray) ssh_keys
        = _snapd_json_get_array (object, "ssh-keys");
    ...
    if (json_object_has_member (object, "macaroon")) {
        ...
        for (guint i = 0; i < json_array_get_length (discharges); i++) {
            ...
            g_ptr_array_add (discharge_array,
                             (gpointer)json_node_get_string (node));
        }
        g_ptr_array_add (discharge_array, NULL);
        auth_data = snapd_auth_data_new (
            _snapd_json_get_string (object, "macaroon", NULL),
            (GStrv)discharge_array->pdata);
    }

    return g_object_new (
        SNAPD_TYPE_USER_INFORMATION, "id",
        _snapd_json_get_int (object, "id", -1), "username",
        _snapd_json_get_string (object, "username", NULL), "email",
...
_snapd_json_parse_slot (JsonNode *node, GError **error)
{
    ...
    return g_object_new (
        SNAPD_TYPE_SLOT, "name", _snapd_json_get_string (object, "slot", NULL),
        "snap", _snapd_json_get_string (object, "snap", NULL), "interface",
        _snapd_json_get_string (object, "interface", NULL), "label",
        _snapd_json_get_string (object, "label", NULL), "connections",
        plug_refs, "attributes", attributes,
        // FIXME: apps
        NULL);
}
...
_snapd_json_parse_plug (JsonNode *node, GError **error)
{
    ...
    return g_object_new (
        SNAPD_TYPE_PLUG, "name", _snapd_json_get_string (object, "plug", NULL),
        "snap", _snapd_json_get_string (object, "snap", NULL), "interface",
        _snapd_json_get_string (object, "interface", NULL), "label",
        _snapd_json_get_string (object, "label", NULL), "connections",
        slot_refs, "attributes", attributes,
        // FIXME: apps
        NULL);
}
...
_snapd_json_parse_connection (JsonNode *node, GError **error)
{
    ...
    if (json_object_has_member (object, "slot-attrs")) {
        ...
        slot_attributes = g_hash_table_new_full (
            g_str_hash, g_str_equal, g_free, (GDestroyNotify)g_variant_unref);
    ...
    if (json_object_has_member (object, "plug-attrs")) {
        ...
        plug_attributes = g_hash_table_new_full (
            g_str_hash, g_str_equal, g_free, (GDestroyNotify)g_variant_unref);
...
_snapd_json_parse_interface (JsonNode *node, GError **error)
{
    ...
    return g_object_new (
        SNAPD_TYPE_INTERFACE, "name",
        _snapd_json_get_string (object, "name", NULL), "summary",
        _snapd_json_get_string (object, "summary", NULL), "doc-url",
        _snapd_json_get_string (object, "doc-url", NULL), "plugs", plug_array,
        "slots", slot_array, NULL);
}
