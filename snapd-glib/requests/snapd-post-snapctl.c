...
SnapdPostSnapctl *
_snapd_post_snapctl_new (const gchar *context_id,
                         GStrv args,
                         GCancellable *cancellable,
                         GAsyncReadyCallback callback,
                         gpointer user_data)
{
    SnapdPostSnapctl *self = SNAPD_POST_SNAPCTL (g_object_new (
        snapd_post_snapctl_get_type (), "cancellable", cancellable,
...
generate_post_snapctl_request (SnapdRequest *request, GBytes **body)
{
    ...
    SoupMessage *message
        = soup_message_new ("POST", "http://snapd/v2/snapctl");
                             ...
                             GError **error)
{
    ...
    g_autoptr (JsonObject) response = _snapd_json_parse_response (
        content_type, body, maintenance, &error_value, error);
    if (response == NULL) {
        if (g_error_matches (*error, SNAPD_ERROR, SNAPD_ERROR_UNSUCCESSFUL)
            && error_value != NULL
            && json_node_get_value_type (error_value) == JSON_TYPE_OBJECT) {
            JsonObject *object = json_node_get_object (error_value);

            self->stdout_output
                = g_strdup (_snapd_json_get_string (object, "stdout", NULL));
            self->stderr_output
                = g_strdup (_snapd_json_get_string (object, "stderr", NULL));
    ...
    self->stdout_output
        = g_strdup (_snapd_json_get_string (result, "stdout", NULL));
    self->stderr_output
        = g_strdup (_snapd_json_get_string (result, "stderr", NULL));
