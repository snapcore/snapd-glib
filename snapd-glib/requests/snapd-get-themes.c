...
SnapdGetThemes *
_snapd_get_themes_new (GStrv gtk_theme_names,
                       GStrv icon_theme_names,
                       GStrv sound_theme_names,
                       ...
                       gpointer user_data)
{
    SnapdGetThemes *self = SNAPD_GET_THEMES (g_object_new (
        snapd_get_themes_get_type (), "cancellable", cancellable,
...
generate_get_themes_request (SnapdRequest *request, GBytes **body)
{
    ...
    g_autoptr (GString) path
        = g_string_new ("http://snapd/v2/accessories/themes");
    gboolean first_param = TRUE;

    add_theme_names (path, &first_param, "gtk-theme=", self->gtk_theme_names);
    add_theme_names (path, &first_param,
                     "icon-theme=", self->icon_theme_names);
    add_theme_names (path, &first_param,
                     "sound-theme=", self->sound_theme_names);
...
static gboolean
parse_get_themes_response (SnapdRequest *request,
                           guint status_code,
