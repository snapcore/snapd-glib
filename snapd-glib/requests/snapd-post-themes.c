...
G_DEFINE_TYPE (SnapdPostThemes,
               snapd_post_themes,
               snapd_request_async_get_type ())

SnapdPostThemes *
_snapd_post_themes_new (GStrv gtk_theme_names,
                        GStrv icon_theme_names,
                        GStrv sound_theme_names,
                        ...
                        gpointer user_data)
{
    SnapdPostThemes *self = SNAPD_POST_THEMES (g_object_new (
        snapd_post_themes_get_type (), "cancellable", cancellable,
        "ready-callback", callback, "ready-callback-data", user_data,
        "progress-callback", progress_callback, "progress-callback-data",
        progress_callback_data, "change-api-path", "/v2/accessories/changes",
        NULL));
