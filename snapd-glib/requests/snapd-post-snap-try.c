...
G_DEFINE_TYPE (SnapdPostSnapTry,
               snapd_post_snap_try,
...
_snapd_post_snap_try_new (const gchar *path,
                          ...
                          gpointer user_data)
{
    SnapdPostSnapTry *self = SNAPD_POST_SNAP_TRY (g_object_new (
        snapd_post_snap_try_get_type (), "cancellable", cancellable,
...
generate_post_snap_try_request (SnapdRequest *request, GBytes **body)
{
    ...
    soup_multipart_to_message (
        multipart, soup_message_get_request_headers (message), body);
#else
