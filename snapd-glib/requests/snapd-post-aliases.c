...
G_DEFINE_TYPE (SnapdPostAliases,
               snapd_post_aliases,
...
_snapd_post_aliases_new (const gchar *action,
                         ...
                         gpointer user_data)
{
    SnapdPostAliases *self = SNAPD_POST_ALIASES (g_object_new (
        snapd_post_aliases_get_type (), "cancellable", cancellable,
...
generate_post_aliases_request (SnapdRequest *request, GBytes **body)
{
    ...
    SoupMessage *message
        = soup_message_new ("POST", "http://snapd/v2/aliases");
