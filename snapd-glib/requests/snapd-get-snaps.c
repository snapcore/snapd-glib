...
SnapdGetSnaps *
_snapd_get_snaps_new (GCancellable *cancellable,
                      GStrv names,
                      GAsyncReadyCallback callback,
                      gpointer user_data)
{
    SnapdGetSnaps *self = SNAPD_GET_SNAPS (g_object_new (
        snapd_get_snaps_get_type (), "cancellable", cancellable,
...
generate_get_snaps_request (SnapdRequest *request, GBytes **body)
{
    ...
    if (self->names != NULL) {
        ...
        g_ptr_array_add (query_attributes,
                         g_strdup_printf ("snaps=%s", names_list));
    }
