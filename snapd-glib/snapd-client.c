...
request_data_unref (RequestData *data)
{
    ...
    if (data->cancelled_id != 0)
        g_cancellable_disconnect (
            _snapd_request_get_cancellable (data->request),
            data->cancelled_id);
    data->cancelled_id = 0;
...
static void
complete_request_unlocked (SnapdClient *self,
                           SnapdRequest *request,
                           GError *error)
{
    SnapdClientPrivate *priv = snapd_client_get_instance_private (self);
...
complete_request (SnapdClient *self, SnapdRequest *request, GError *error)
{
    ...
    g_autoptr (GMutexLocker) locker
        = g_mutex_locker_new (&priv->requests_mutex);
...
complete_all_requests (SnapdClient *self, GError *error)
{
    ...
    /* Cancel synchronous requests (we'll never know the result); reschedule
     * async ones (can reconnect to check result) */
    g_autoptr (GPtrArray) requests_copy
        = g_ptr_array_new_with_free_func ((GDestroyNotify)request_data_unref);
    for (guint i = 0; i < priv->requests->len; i++)
        g_ptr_array_add (requests_copy, request_data_ref (g_ptr_array_index (
                                            priv->requests, i)));
...
static gboolean
read_chunk_header (const gchar *body,
                   gsize body_length,
...
update_changes (SnapdClient *self, SnapdChange *change, JsonNode *data)
{
    ...
    if (snapd_change_get_ready (change)) {
        ...
        if (g_cancellable_set_error_if_cancelled (
                _snapd_request_get_cancellable (SNAPD_REQUEST (request)),
                &error)) {
            complete_request (self, SNAPD_REQUEST (request), error);
            return;
        }
...
static void
parse_response (SnapdClient *self,
                SnapdRequest *request,
                guint status_code,
                ...
                GBytes *body)
{
    ...
    if (!SNAPD_REQUEST_GET_CLASS (request)->parse_response (
        ...
        if (SNAPD_IS_GET_CHANGE (request)) {
            complete_change (
                self,
                _snapd_get_change_get_change_id (SNAPD_GET_CHANGE (request)),
                error);
            complete_request (self, request, error);
        } else if (SNAPD_IS_POST_CHANGE (request)) {
            complete_change (
                self,
                _snapd_post_change_get_change_id (SNAPD_POST_CHANGE (request)),
                error);
            complete_request (self, request, error);
        } else
            complete_request (self, request, error);
    ...
    if (SNAPD_IS_GET_CHANGE (request))
        update_changes (
            self, _snapd_get_change_get_change (SNAPD_GET_CHANGE (request)),
            _snapd_get_change_get_data (SNAPD_GET_CHANGE (request)));
    else if (SNAPD_IS_POST_CHANGE (request))
        update_changes (
            self, _snapd_post_change_get_change (SNAPD_POST_CHANGE (request)),
    ...
    if (SNAPD_IS_REQUEST_ASYNC (request)) {
        ...
        if (g_cancellable_is_cancelled (
                _snapd_request_get_cancellable (request)))
...
read_cb (GSocket *socket, GIOCondition condition, SnapdClient *self)
{
    ...
    g_byte_array_set_size (priv->buffer,
                           orig_length + (n_read >= 0 ? n_read : 0));

    if (n_read == 0) {
        g_autoptr (GError) e = g_error_new (
            SNAPD_ERROR, SNAPD_ERROR_READ_FAILED, "snapd connection closed");
    ...
    while (TRUE) {
        ...
        if (priv->response_headers == NULL) {
            gchar *body = g_strstr_len ((gchar *)priv->buffer->data,
                                        priv->buffer->len, "\r\n\r\n");
            if (body == NULL)
                return G_SOURCE_CONTINUE;
            body += 4;
            ...
            if (!soup_headers_parse_response (
                    (gchar *)priv->buffer->data, header_length,
                    priv->response_headers, NULL, &priv->response_status_code,
                    NULL)) {
                g_autoptr (GError) e
                    = g_error_new (SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
        ...
        case SOUP_ENCODING_CHUNKED:
            while (offset < priv->buffer->len
                   && read_chunk_header (
                       (const gchar *)priv->buffer->data + offset,
        ...
        if (g_strcmp0 (content_type, "application/json-seq") == 0) {
            ...
            while (priv->response_body->len > 0) {
                ...
                gboolean have_end
                    = seq_end < priv->response_body->len
                      && priv->response_body->data[seq_end] == 0x1e;
                ...
                if (!parse_seq (self, request,
                                (const gchar *)priv->response_body->data
                                    + seq_start,
                                seq_end - seq_start, &json_error)) {
                    g_warning ("Ignoring invalid JSON: %s",
                               json_error->message);
                    return G_SOURCE_REMOVE;
                }
            ...
            g_autoptr (GBytes) b = g_bytes_new (priv->response_body->data,
                                                priv->response_body->len);
            parse_response (self, request, priv->response_status_code,
                            content_type, b);
        }

        if (is_complete) {
#if SOUP_CHECK_VERSION(2, 99, 2)
            g_clear_pointer (&priv->response_headers,
...
request_cancelled_cb (GCancellable *cancellable, RequestData *data)
{
    ...
    if (SNAPD_IS_REQUEST_ASYNC (data->request)) {
        ...
        /* Execute in an idle thread so g_cancellable_disconnect doesn't
         * deadlock */
        g_autoptr (GSource) idle_source = g_idle_source_new ();
        g_source_set_callback (idle_source, cancel_idle_cb,
                               request_data_ref (data),
                               (GDestroyNotify)request_data_unref);
        g_source_attach (idle_source,
                         _snapd_request_get_context (data->request));
    }
}

static GSocket *
open_snapd_socket (const gchar *socket_path,
...
static gboolean
write_to_snapd (SnapdClient *self,
                GByteArray *data,
                GCancellable *cancellable,
                GError **error)
{
    ...
    while (n_sent < data->len) {
        gssize n_written = g_socket_send (
            priv->snapd_socket, (const gchar *)(data->data + n_sent),
...
send_request (SnapdClient *self, SnapdRequest *request)
{
    ...
    if (priv->allow_interaction)
        soup_message_headers_append (request_headers, "X-Allow-Interaction",
                                     "true");
...
follow_logs_data_new (SnapdClient *client,
                      SnapdLogCallback callback,
                      gpointer callback_data)
{
 ...
 */
gboolean
snapd_client_connect_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
 ...
 */
void
snapd_client_set_allow_interaction (SnapdClient *self,
                                    gboolean allow_interaction)
{
    SnapdClientPrivate *priv = snapd_client_get_instance_private (self);
    g_return_if_fail (SNAPD_IS_CLIENT (self));
...
snapd_client_login_async (SnapdClient *self,
                          ...
                          gpointer user_data)
{
    snapd_client_login2_async (self, email, password, otp, cancellable,
                               callback, user_data);
}
 ...
 */
SnapdAuthData *
snapd_client_login_finish (SnapdClient *self,
                           GAsyncResult *result,
                           GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
...
snapd_client_login2_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
gboolean
snapd_client_logout_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 * @client: a #SnapdClient.
 * @since_date_time: send only the notices generated after this moment (NULL
 * for all).
 ...
 * microseconds, so, if nanosecond resolution is needed, it is mandatory to
 * call #snapd_client_notices_set_after_notice before calling this method.
 *
 * Since: 1.65
 */
void
                                ...
                                gpointer user_data)
{
    ...
    snapd_client_get_notices_with_filters_async (
        self, NULL, NULL, NULL, NULL, since_date_time, timeout, cancellable,
        callback, user_data);
}
 ...
 * object containing the requested notices, or NULL in case of error.
 *
 * Since: 1.65
 */
GPtrArray *
snapd_client_get_notices_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
GPtrArray *
snapd_client_get_notices_with_filters_finish (SnapdClient *self,
                                              GAsyncResult *result,
                                              GError **error)
{
 ...
 * an accuracy of 1 microsecond, but to receive notice events correctly,
 * without loosing any of them, an accuracy of 1 nanosecond is needed in the
 * value passed on in the @since_date_time parameter.
 *
 ...
 * call to get the notices. This is currently needed because GDateTime has only
 * an accuracy of 1 microsecond, but to receive notice events correctly,
 * without loosing any, it is needed 1 nanosecond accuracy in the value passed
 ...
 */
void
snapd_client_notices_set_since_nanoseconds (SnapdClient *self,
 ...
 */
GPtrArray *
snapd_client_get_changes_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
SnapdChange *
snapd_client_get_change_finish (SnapdClient *self,
                                GAsyncResult *result,
                                GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
                                 ...
                                 gpointer user_data)
{
    ...
    g_autoptr (SnapdPostChange) request = _snapd_post_change_new (
        id, "abort", cancellable, callback, user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
SnapdChange *
snapd_client_abort_change_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
...
snapd_client_get_system_information_finish (SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
    ...
    if (!_snapd_request_propagate_error (SNAPD_REQUEST (request), error))
        return NULL;
    return g_object_ref (
        _snapd_get_system_info_get_system_information (request));
}
 ...
 */
SnapdSnap *
snapd_client_list_one_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
 ...
 */
SnapdSnap *
snapd_client_get_snap_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
                                  ...
                                  gpointer user_data)
{
    ...
    g_autoptr (SnapdGetSnapConf) request = _snapd_get_snap_conf_new (
        name, keys, cancellable, callback, user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
GHashTable *
snapd_client_get_snap_conf_finish (SnapdClient *self,
                                   GAsyncResult *result,
                                   GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
gboolean
snapd_client_set_snap_conf_finish (SnapdClient *self,
                                   GAsyncResult *result,
                                   GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
...
snapd_client_get_apps_async (SnapdClient *self,
                             ...
                             gpointer user_data)
{
    return snapd_client_get_apps2_async (self, flags, NULL, cancellable,
                                         callback, user_data);
}
 ...
 */
GPtrArray *
snapd_client_get_apps_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
 ...
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdGetAppsFlags to control what results are returned.
 * @snaps: (allow-none): A list of snap names to return results for. If %NULL
 * or empty then apps for all installed snaps are returned.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 */
GPtrArray *
snapd_client_get_apps2_finish (SnapdClient *self,
                               GAsyncResult *result,
                               GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
SnapdIcon *
snapd_client_get_icon_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
...
snapd_client_list_async (SnapdClient *self,
                         ...
                         gpointer user_data)
{
    return snapd_client_get_snaps_async (self, SNAPD_GET_SNAPS_FLAGS_NONE,
                                         NULL, cancellable, callback,
                                         user_data);
}
 ...
 */
GPtrArray *
snapd_client_list_finish (SnapdClient *self,
                          GAsyncResult *result,
                          GError **error)
{
 ...
 * @names: (allow-none): A list of snap names to return results for. If %NULL
 * or empty then all installed snaps are returned.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 */
GPtrArray *
snapd_client_get_snaps_finish (SnapdClient *self,
                               GAsyncResult *result,
                               GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 * See snapd_client_get_assertions_sync() for more information.
 *
 * Returns: (transfer full) (array zero-terminated=1): an array of assertions
 * or %NULL on error.
 *
 * Since: 1.8
 */
GStrv
snapd_client_get_assertions_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
                                   ...
                                   gpointer user_data)
{
    ...
    g_autoptr (SnapdPostAssertions) request = _snapd_post_assertions_new (
        assertions, cancellable, callback, user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_add_assertions_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_get_interfaces_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GPtrArray **plugs,
                                    GPtrArray **slots,
                                    GError **error)
{
    ...
    if (!_snapd_request_propagate_error (SNAPD_REQUEST (request), error))
        return FALSE;
    if (plugs)
        *plugs = g_ptr_array_ref (
            _snapd_get_interfaces_legacy_get_plugs (request));
    if (slots)
        *slots = g_ptr_array_ref (
            _snapd_get_interfaces_legacy_get_slots (request));
    return TRUE;
}
                                    ...
                                    gpointer user_data)
{
    snapd_client_get_connections2_async (
        self, SNAPD_GET_CONNECTIONS_FLAGS_NONE, NULL, NULL, cancellable,
        callback, user_data);
}
 ...
 */
gboolean
snapd_client_get_connections_finish (SnapdClient *self,
                                     GAsyncResult *result,
                                     ...
                                     GError **error)
{
    return snapd_client_get_connections2_finish (
        self, result, established, undesired, plugs, slots, error);
}
 ...
 * returned.
 * @snap: (allow-none): the name of the snap to get connections for or %NULL
 * for all snaps.
 * @interface: (allow-none): the name of the interface to get connections for
 * or %NULL for all interfaces.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 */
gboolean
snapd_client_get_connections2_finish (SnapdClient *self,
                                      GAsyncResult *result,
                                      GPtrArray **established,
                                      GPtrArray **undesired,
                                      GPtrArray **plugs,
                                      ...
                                      GError **error)
{
    ...
    if (!_snapd_request_propagate_error (SNAPD_REQUEST (request), error))
        return FALSE;
    if (established)
        *established = g_ptr_array_ref (
            _snapd_get_connections_get_established (request));
    if (undesired)
        *undesired
            = g_ptr_array_ref (_snapd_get_connections_get_undesired (request));
    if (plugs)
                                      ...
                                      gpointer user_data)
{
    ...
    g_autoptr (SnapdPostInterfaces) request = _snapd_post_interfaces_new (
        "connect", plug_snap, plug_name, slot_snap, slot_name,
 ...
 * @query: (allow-none): query string to send or %NULL to return featured
 * snaps.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 */
GPtrArray *
snapd_client_find_finish (SnapdClient *self,
                          GAsyncResult *result,
 ...
 * sections.
 * @query: (allow-none): query string to send or %NULL to get all snaps from
 * the given section.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 */
GPtrArray *
snapd_client_find_section_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  gchar **suggested_currency,
                                  GError **error)
{
 ...
 */
GPtrArray *
snapd_client_find_category_finish (SnapdClient *self,
                                   GAsyncResult *result,
                                   gchar **suggested_currency,
                                   GError **error)
{
 ...
 */
GPtrArray *
snapd_client_find_refreshable_finish (SnapdClient *self,
                                      GAsyncResult *result,
                                      GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
                            ...
                            gpointer user_data)
{
    snapd_client_install2_async (
        self, SNAPD_INSTALL_FLAGS_NONE, name, channel, NULL, progress_callback,
 ...
 */
gboolean
snapd_client_install_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
                             ...
                             gpointer user_data)
{
    ...
    g_autoptr (SnapdPostSnap) request = _snapd_post_snap_new (
        name, "install", progress_callback, progress_callback_data,
        cancellable, callback, user_data);
 ...
 */
gboolean
snapd_client_install2_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
...
install_stream_data_new (SnapdClient *client,
                         SnapdPostSnapStream *request,
                         GCancellable *cancellable,
                         GInputStream *stream)
{
...
static void
stream_read_cb (GObject *source_object,
                GAsyncResult *result,
                gpointer user_data)
{
    ...
    if (!_snapd_request_propagate_error (SNAPD_REQUEST (data->request),
                                         &error))
        return;
    ...
    if (g_bytes_get_size (read_data) == 0)
        ...
        _snapd_post_snap_stream_append_data (
            data->request, g_bytes_get_data (read_data, NULL),
                                   ...
                                   gpointer user_data)
{
    ...
    g_autoptr (SnapdPostSnapStream) request = _snapd_post_snap_stream_new (
        progress_callback, progress_callback_data, cancellable, callback,
        user_data);
    if ((flags & SNAPD_INSTALL_FLAGS_CLASSIC) != 0)
 ...
 */
gboolean
snapd_client_install_stream_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
                        ...
                        gpointer user_data)
{
    ...
    g_autoptr (SnapdPostSnapTry) request = _snapd_post_snap_try_new (
        path, progress_callback, progress_callback_data, cancellable, callback,
        user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_try_finish (SnapdClient *self,
                         GAsyncResult *result,
                         GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
                            ...
                            gpointer user_data)
{
    ...
    g_autoptr (SnapdPostSnap) request = _snapd_post_snap_new (
        name, "refresh", progress_callback, progress_callback_data,
        cancellable, callback, user_data);
    _snapd_post_snap_set_channel (request, channel);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_refresh_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
GStrv
snapd_client_refresh_all_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_remove_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
 ...
 */
gboolean
snapd_client_remove2_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_enable_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
                            ...
                            gpointer user_data)
{
    ...
    g_autoptr (SnapdPostSnap) request = _snapd_post_snap_new (
        name, "disable", progress_callback, progress_callback_data,
        cancellable, callback, user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_disable_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_switch_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_check_buy_finish (SnapdClient *self,
                               GAsyncResult *result,
                               GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 * @currency: the currency to buy with as an ISO 4217 currency code, e.g.
 * "NZD".
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 */
gboolean
snapd_client_buy_finish (SnapdClient *self,
                         GAsyncResult *result,
                         GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
                                ...
                                gpointer user_data)
{
    ...
    g_autoptr (SnapdPostCreateUser) request = _snapd_post_create_user_new (
        email, cancellable, callback, user_data);
 ...
 */
SnapdUserInformation *
snapd_client_create_user_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
    ...
    if (!_snapd_request_propagate_error (SNAPD_REQUEST (request), error))
        return NULL;
    return g_object_ref (
        _snapd_post_create_user_get_user_information (request));
}
 ...
 * that are valid for this device. See snapd_client_create_users_sync() for
 * more information.
 *
 * Since: 1.3
 */
 ...
 */
GPtrArray *
snapd_client_create_users_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
GPtrArray *
snapd_client_get_users_finish (SnapdClient *self,
                               GAsyncResult *result,
                               GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 * See snapd_client_get_sections_sync() for more information.
 *
 * Returns: (transfer full) (array zero-terminated=1): an array of section
 ...
 */
GStrv
snapd_client_get_sections_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
GPtrArray *
snapd_client_get_categories_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
GPtrArray *
snapd_client_get_aliases_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
gboolean
snapd_client_alias_finish (SnapdClient *self,
                           GAsyncResult *result,
                           GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_unalias_finish (SnapdClient *self,
                             GAsyncResult *result,
                             GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_prefer_finish (SnapdClient *self,
                            GAsyncResult *result,
                            GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
                                   ...
                                   gpointer user_data)
{
    ...
    g_task_return_new_error (
        task, SNAPD_ERROR, SNAPD_ERROR_FAILED,
 ...
 */
gboolean
snapd_client_enable_aliases_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_disable_aliases_finish (SnapdClient *self,
                                     GAsyncResult *result,
                                     GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
gboolean
snapd_client_reset_aliases_finish (SnapdClient *self,
                                   GAsyncResult *result,
                                   GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
...
snapd_client_run_snapctl_async (SnapdClient *self,
                                ...
                                gpointer user_data)
{
    return snapd_client_run_snapctl2_async (self, context_id, args,
                                            cancellable, callback, user_data);
}
 ...
 * @client: a #SnapdClient.
 * @result: a #GAsyncResult.
 * @stdout_output: (out) (allow-none): the location to write the stdout from
 * the command or %NULL.
 * @stderr_output: (out) (allow-none): the location to write the stderr from
 * the command or %NULL.
 * @error: (allow-none): #GError location to store the error occurring, or
 * %NULL to ignore.
 *
 ...
 */
gboolean
snapd_client_run_snapctl_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 gchar **stdout_output,
                                 gchar **stderr_output,
                                 GError **error)
 ...
 */
gboolean
snapd_client_run_snapctl2_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  gchar **stdout_output,
                                  gchar **stderr_output,
                                  int *exit_code,
                                  GError **error)
{
    ...
    if (stdout_output)
        *stdout_output
            = g_strdup (_snapd_post_snapctl_get_stdout_output (request));
    if (stderr_output)
        *stderr_output
            = g_strdup (_snapd_post_snapctl_get_stderr_output (request));
    if (exit_code)
 ...
 */
GBytes *
snapd_client_download_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
gboolean
snapd_client_check_themes_finish (SnapdClient *self,
                                  GAsyncResult *result,
                                  ...
                                  GError **error)
{
    ...
    if (gtk_theme_status)
        *gtk_theme_status = g_hash_table_ref (
            _snapd_get_themes_get_gtk_theme_status (request));
    if (icon_theme_status)
        *icon_theme_status = g_hash_table_ref (
            _snapd_get_themes_get_icon_theme_status (request));
    if (sound_theme_status)
        *sound_theme_status = g_hash_table_ref (
            _snapd_get_themes_get_sound_theme_status (request));
 ...
 */
void
snapd_client_install_themes_async (SnapdClient *self,
                                   GStrv gtk_theme_names,
                                   GStrv icon_theme_names,
                                   GStrv sound_theme_names,
                                   SnapdProgressCallback progress_callback,
                                   ...
                                   gpointer user_data)
{
    ...
    g_autoptr (SnapdPostThemes) request = _snapd_post_themes_new (
        gtk_theme_names, icon_theme_names, sound_theme_names,
        progress_callback, progress_callback_data, cancellable, callback,
        user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_install_themes_finish (SnapdClient *self,
                                    GAsyncResult *result,
                                    GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
 ...
 */
GPtrArray *
snapd_client_get_logs_finish (SnapdClient *self,
                              GAsyncResult *result,
                              GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), NULL);
 ...
 */
void
snapd_client_follow_logs_async (SnapdClient *self,
                                GStrv names,
                                ...
                                gpointer user_data)
{
    ...
    g_autoptr (SnapdGetLogs) request = _snapd_get_logs_new (
        ...
        (GDestroyNotify)follow_logs_data_free, cancellable, callback,
        user_data);
    send_request (self, SNAPD_REQUEST (request));
}
 ...
 */
gboolean
snapd_client_follow_logs_finish (SnapdClient *self,
                                 GAsyncResult *result,
                                 GError **error)
{
    g_return_val_if_fail (SNAPD_IS_CLIENT (self), FALSE);
