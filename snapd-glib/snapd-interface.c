...
snapd_interface_make_label (SnapdInterface *self)
{
    if (strcmp (self->name, "account-control") == 0)
        ...
        return g_strdup (
            _ ("Provide entropy to hardware random number generator"));
        ...
        return g_strdup (_ ("Access the NetworkManager service to read and "
        ...
        return g_strdup (_ ("Access the ofono service to read and change "
                            "network settings for mobile telephony"));
        ...
        return g_strdup (_ ("Access the UDisks2 service for configuring disks "
                            "and removable media"));
    else if (strcmp (self->name, "unity8-calendar") == 0)
        return g_strdup (
            _ ("Read/change shared calendar events in Ubuntu Unity 8"));
...
snapd_interface_class_init (SnapdInterfaceClass *klass)
{
    ...
    g_object_class_install_property (
        ...
        g_param_spec_string ("doc-url", "doc-url",
                             "Interface documentation URL", NULL,
                             G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property (
